...
def render_investment_section(suffix):
    ...
    # Nota de advertencia superior
    st.caption("⚠️ Los valores mostrados son proyecciones de mercado. El arbitraje comercial conlleva riesgos de fluctuación de precios.")

    # Base de Datos con IDs FIJOS y 24 productos
    oportunidades = [
        # --- HARDWARE (IDs 100+) ---
        ...
        {"id": "305", "n": "Nintendo Switch OLED", "c": 290, "v": 385, "r": "MEDIO", "clr": "#facc15", "q": "nintendo switch oled", "cat": "🎮 GAMING"},
        {"id": "306", "n": "Steam Deck 512GB OLED", "c": 540, "v": 690, "r": "ALTO", "clr": "#ef4444", "q": "steam deck oled", "cat": "🎮 GAMING"},

        # --- GADGETS & AUDIO (IDs 400+) ---
    ...
    for i, op in enumerate(items):
        with cols[i % 4]:
            ...
            st.markdown(f"""
                <div style="background:#fff; color:#000; padding:12px; border-radius:10px; border-top:5px solid {op['clr']}; margin-bottom:15px; box-shadow:0 2px 5px rgba(0,0,0,0.1); min-height:235px; display:flex; flex-direction:column; justify-content:space-between;">
                    
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; width:100%; margin-bottom:10px; gap:5px;">
                        <span style="font-size:7.5px; font-weight:900; color:{op['clr']}; text-transform:uppercase;">{op['cat']}</span>
                        <span style="font-size:8px; color:#94a3b8; font-family:monospace; flex-shrink:0;">{ref_id}</span>
                    </div>
                    
                    <h4 style="margin:0 0 10px 0; font-size:11px; line-height:1.2; height:28px; overflow:hidden; color:#1e293b; font-weight:700;">{op['n']}</h4>
                    
                    <div style="background:#f8fafc; padding:8px; border-radius:6px; margin-bottom:10px; border:1px solid #f1f5f9;">
                        <div style="display:flex; justify-content:space-between; font-size:10px; margin-bottom:2px;">
                            <span style="color:#64748b;">ROI Proyectado:</span><b style="color:#16a34a;">+{roi}%</b>
                        </div>
                        <div style="display:flex; justify-content:space-between; font-size:10px;">
                            <span style="color:#64748b;">Margen Est. (*):</span><b style="color:#1e40af;">+${margen}</b>
                        </div>
                    </div>
                    ...
                    <div style="margin-top:auto;">
                        ...
                        <p style="font-size:7.5px; color:#94a3b8; margin:4px 0 0 0; text-align:right; font-weight:600;">NODOS: {stock} Ltes</p>
                    </div>
                    
                    <a href="{amz_url}" target="_blank" style="background:#fbbf24; color:#000; text-decoration:none; display:block; text-align:center; padding:8px; border-radius:5px; font-weight:800; font-size:10px; border-bottom:2px solid #d97706; margin-top:8px;">ADQUIRIR NODO</a>
                </div>
            """, unsafe_allow_html=True)
    
    st.markdown("<p style='font-size:10px; color:#94a3b8; text-align:center;'>(*) Los márgenes son brutos y no incluyen impuestos locales o costos de envío final.</p>", unsafe_allow_html=True)
