...
def render_crypto_section(lista_crypto):
    ...
    for i, c in enumerate(lista_crypto):
        with cols[i % 3]:
            ex_html = "".join([f'<div style="display:flex; justify-content:space-between; font-size: 12px; margin: 4px 0;"><span>{ex["name"]}</span><b style="color:{"#22c55e" if ex["type"]=="COMPRA" else "#ef4444" if ex["type"]=="VENTA" else "white"}">${ex["price"]}</b></div>' for ex in c['exchanges']])
            
            st.markdown(f"""
            <div style="background: #1e293b; border-radius: 10px; padding: 15px; border: 1px solid #334155; color: white;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <h3 style="color: gold; margin: 0;">{c['coin']}</h3>
                    <span style="background: #064e3b; color: #4ade80; padding: 2px 8px; border-radius: 5px; font-size: 12px;">ROI: {c['roi']}%</span>
                </div>
                <p style="font-size: 20px; font-weight: bold; color: #22c55e; margin: 10px 0;">Gap: ${c['gap']}</p>
